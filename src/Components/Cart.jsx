    ...
    return (
      <div className="empty-container">
        <div className="grplink">
          <label className="emp">----Cart Empty----</label>
          <br></br>
          <Link className="emplink" to="/login">
            Login
          </Link>
        </div>
      </div>
    ...
    return (
      <>
              ...
              <div key={index}>
                <div className="carthold">
                  <Link to={`/productdetails/${item.id}`} className="llink">
                    ...
                    <div className="crd-brand col-sm-12">
                      <mark>{item.brand}</mark>
                    </div>
                    <div className="crd-stoke col-sm-12">
                      Stoke-left - <kbd>{item.stoke}</kbd>
                    </div>
                    <div className="crd-model col-sm-12">{item.model}</div>
                    <div className="crd-rate col-sm-12">
                      INR {totalItemPrice}
                    </div>
        ...
        <div className="total-price">
          Total Cart Price:{" "}
          <label className="ttl">INR {calculateTotalPrice()}</label>
        </div>
      </>
    );
  }
